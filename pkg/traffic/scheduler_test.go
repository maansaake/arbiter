...
func TestRunAndAwaitStop(t *testing.T) {
	...
	err := Run(ctx, module.Modules{testmod}, &report.YAMLReporter{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunNoOps(t *testing.T) {
	err := Run(context.TODO(), module.Modules{testmodule.NewTestModule()}, nil)
...
func TestRunZeroRate(t *testing.T) {
	...
	err := Run(context.TODO(), module.Modules{mod}, nil)
